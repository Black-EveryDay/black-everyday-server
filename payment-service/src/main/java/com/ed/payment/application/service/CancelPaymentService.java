...
public class CancelPaymentService implements CancelPaymentUseCase {
  ...
  public void cancelPayment(OrderPaymentCancelRequest request) {
    Payment payment = readPaymentPort.getCancelablePayment(request.getPaymentId(), request.getRequestDateTime());
    PaymentCanceled paymentCanceled = cancelPaymentPort.cancelPayment(
        payment.getPaymentKey(), payment.getIdempotencyKey(), request.getCancelReason());

    updatePaymentPort.updatePaymentStatusById(payment.getPaymentId(), paymentCanceled.getPaymentStatus());
