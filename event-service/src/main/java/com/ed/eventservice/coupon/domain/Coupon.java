...
public class Coupon {
  ...
  public void useCoupon(UUID userId, UUID brandId, UUID productId, UUID orderId) {
    ...
    this.couponStatusChangelogs.add(CouponStatusChangeLog.builder()
        .before(this.state)
        .after(CouponState.REDEEMED)
  ...
  private void checkProductCouponUsageTarget(UUID productId) {
    if (!this.couponTemplate.getCouponUsageTargetInfo().getCouponUsageTargetId()
        .equals(productId)) {
      throw new DomainException(ExceptionStatus.COUPON_USAGE_TARGET_NOT_MATCHED);
    }
  }
